...
struct ApiConfig {
    ...
    uint16_t    port           = 8080;
    std::string cors_origin    = "";    // must be set explicitly; empty blocks credentialed CORS
    std::string hec_token;
    bool        secure_cookies = true;  // requires HTTPS; set false only for local HTTP dev
};
